...
export function Topbar() {
  const {
    theme, toggleTheme, setCommandOpen, setNotifPanelOpen, currentRole,
    setRole, currentBranchId, setBranch, setView
  } = useAppStore();
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-border/60 bg-background/80 px-4 md:px-6 backdrop-blur-xl">
      {/* Search */}
      <button
        onClick={() => setCommandOpen(true)}
        className="group flex h-10 flex-1 max-w-md items-center gap-2.5 rounded-2xl bg-muted/60 px-3.5 ring-1 ring-border/60 transition-all hover:bg-muted hover:ring-border"
      >
        <Search className="h-4 w-4 text-muted-foreground" />
        <span className="text-sm text-muted-foreground">Search patients, appointments, invoices…</span>
        <kbd className="ml-auto hidden items-center gap-0.5 rounded-md bg-background px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground ring-1 ring-border sm:flex">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="hidden lg:flex items-center gap-2 rounded-2xl bg-muted/40 px-3 py-2 ring-1 ring-border/60">
        ...
        <Popover.Root open={branchMenuOpen} onOpenChange={setBranchMenuOpen}>
          <Popover.Trigger asChild>
            <button className="flex h-10 items-center gap-2 rounded-2xl bg-muted/40 px-3 ring-1 ring-border/60 transition-all hover:bg-muted hover:ring-border">
          ...
          <Popover.Portal>
            <Popover.Content
              align="end"
              sideOffset={8}
              className="z-50 w-56 rounded-2xl bg-popover p-2 premium-shadow-lg ring-1 ring-border"
            >
        ...
        <Popover.Root open={roleMenuOpen} onOpenChange={setRoleMenuOpen}>
          <Popover.Trigger asChild>
            <button className="flex h-10 items-center gap-2 rounded-2xl bg-muted/40 px-3 ring-1 ring-border/60 transition-all hover:bg-muted hover:ring-border">
              ...
              <span className="hidden md:block text-xs font-medium text-foreground">{currentRoleOpt.label}</span>
              <ChevronDown className="h-3.5 w-3.5 text-muted-foreground" />
            </button>
          </Popover.Trigger>
          <Popover.Portal>
            <Popover.Content
              align="end"
              sideOffset={8}
              className="z-50 w-60 rounded-2xl bg-popover p-2 premium-shadow-lg ring-1 ring-border"
            >
              ...
              <button className="flex w-full items-center gap-2.5 rounded-xl px-2.5 py-2 text-sm text-muted-foreground transition-colors hover:bg-muted">
                <SettingsIcon className="h-3.5 w-3.5" /> Account Settings
              </button>
              <button className="flex w-full items-center gap-2.5 rounded-xl px-2.5 py-2 text-sm text-rose-500 transition-colors hover:bg-rose-500/10">
        ...
        <motion.button
          ...
          className="group flex h-10 items-center gap-2 rounded-2xl bg-gradient-to-br from-[#D6F04C] to-[#A3C128] px-4 text-[#0F1117] shadow-[0_8px_24px_-6px_rgba(214,240,76,0.5)] transition-all hover:shadow-[0_12px_32px_-6px_rgba(214,240,76,0.7)]"
        >
